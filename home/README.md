...
| `home/.config/agent-of-empires/config.toml` | `~/.config/agent-of-empires/config.toml` on all OSes (an explicit `install.conf.yaml` link; the Linux `~/.config/**/*` glob also covers it). Adopt-before-relink: an `install.conf.yaml` `shell:` step (after `mise install`, before that link) runs `scripts/bootstrap/adopt-agent-of-empires-config.mjs`, which copies the live file back into this source first when agent-of-empires has replaced the managed symlink with a real file (its atomic config save does this), so the `force: true` relink preserves the tool's change instead of clobbering it. The rest of `~/.config/agent-of-empires/` is machine-local tool state (DBs, tokens, logs) and is NOT tracked |
